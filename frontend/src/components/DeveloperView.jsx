              ...
              <div className="experiment-list">
                {experiments.map((experiment) => {
                  ...
                  if (experimentResults && experimentResults.evaluations) {
                    ...
                    countA = evaluations.filter(
                      (e) => e.preferred_variant === "A"
                    ).length;
                    countB = evaluations.filter(
                      (e) => e.preferred_variant === "B"
                    ).length;

                    const totalAB = countA + countB;

                    percentA = totalAB
                      ? ((countA / totalAB) * 100).toFixed(1)
                      : 0;
                    percentB = totalAB
                      ? ((countB / totalAB) * 100).toFixed(1)
                      : 0;

                    const evalA = evaluations.filter(
                      (e) => e.preferred_variant === "A"
                    );
                    const evalB = evaluations.filter(
                      (e) => e.preferred_variant === "B"
                    );
                    ...
                    if (evalA.length > 0) {
                      ...
                      avgA.comprehension = (
                        evalA.reduce((sum, e) => sum + e.comprehension, 0) /
                        evalA.length
                      ).toFixed(2);
                      avgA.cognitive_load = (
                        evalA.reduce((sum, e) => sum + e.cognitive_load, 0) /
                        evalA.length
                      ).toFixed(2);
                    }
                    ...
                    if (evalB.length > 0) {
                      ...
                      avgB.comprehension = (
                        evalB.reduce((sum, e) => sum + e.comprehension, 0) /
                        evalB.length
                      ).toFixed(2);
                      avgB.cognitive_load = (
                        evalB.reduce((sum, e) => sum + e.cognitive_load, 0) /
                        evalB.length
                      ).toFixed(2);
                    }
                  }
                  ...
                  if (countA > 0 || countB > 0) {
                    ...
                    if (Number(avgA.comprehension) > Number(avgB.comprehension))
                      scoreA += 1;
                    if (Number(avgB.comprehension) > Number(avgA.comprehension))
                      scoreB += 1;

                    if (
                      Number(avgA.cognitive_load) < Number(avgB.cognitive_load)
                    )
                      scoreA += 1;
                    if (
                      Number(avgB.cognitive_load) < Number(avgA.cognitive_load)
                    )
                      scoreB += 1;
                    ...
                    if (scoreA > scoreB) {
                      ...
                      recommendationReason =
                        "obtiene mejor equilibrio entre preferencia, claridad, comprensión y carga cognitiva";
                      ...
                      recommendationReason =
                        "los resultados están equilibrados entre ambas variantes";
                    }
                  }
                      ...
                      {experimentResults && openResults[experiment.id] &&(
                        <div className="results-box">
                          ...
                          <div className="metric-block">
                            <p>
                              <strong>Claridad media:</strong>{" "}
                              {experimentResults.averages?.clarity?.toFixed(2) ||
                                "0.00"}
                            </p>
                            <div className="metric-bar">
                              <div
                                ...
                                style={{
                                  width: `${
                                    (experimentResults.averages?.clarity / 5) * 100
                                  }%`,
                                }}
                              />
                            </div>
                          ...
                          <div className="metric-block">
                            <p>
                              <strong>Comprensión media:</strong>{" "}
                              {experimentResults.averages?.comprehension?.toFixed(
                                2
                              ) || "0.00"}
                            </p>
                            <div className="metric-bar">
                              <div
                                ...
                                style={{
                                  width: `${
                                    (experimentResults.averages?.comprehension / 5) *
                                    100
                                  }%`,
                                }}
                              />
                          ...
                          <div className="metric-block">
                            <p>
                              <strong>Carga cognitiva media:</strong>{" "}
                              {experimentResults.averages?.cognitive_load?.toFixed(
                                2
                              ) || "0.00"}
                            </p>
                            <div className="metric-bar">
                              <div
                                ...
                                style={{
                                  width: `${
                                    (experimentResults.averages?.cognitive_load /
                                      5) *
                                    100
                                  }%`,
                                }}
